    ...
    setup(props) {
        ...
        function handleSubmit() {
           ...
           //validation start
            ...
            if(props.clientCustomField.is_core_field){
                routeName.value = "tables.fields.updateCoreField";
            } else {
                routeName.value = "tables.fields.updateCustomField";
            }
            this.$inertia.post(route(routeName.value, [ props.selectedClientTable.id, props.clientCustomField.id]),form, {
                forceFormData: true,
                onBefore: () => {
