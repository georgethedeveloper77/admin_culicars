...
use Modules\Core\Http\Requests\Menu\StoreCoreMenuRequest;
use Modules\Core\Http\Requests\Menu\UpdateCoreMenuRequest;
use Modules\Core\Http\Services\IconService;
use Modules\Core\Transformers\Backend\Model\Menu\CoreMenuWithKeyResource;

class CoreMenuController extends PsController
{
    private const parentPath = 'menu';
    ...
    public function __construct(protected CoreMenuServiceInterface $coreMenuService,
    ...
    {
        // check permission
        $this->handlePermissionWithModel(CoreMenu::class, Constants::viewAnyAbility);
    ...
    public function create()
    {
        ...
        $this->handlePermissionWithModel(CoreMenu::class, Constants::createAbility);
    ...
    public function store(StoreCoreMenuRequest $request)
    {
        try {
            ...
            // Save Menu Group
            $this->coreMenuService->save(coreMenuData : $validData);
    ...
    {
        // check permission start
        $coreMenu = $this->coreMenuService->get($id);

        $this->handlePermissionWithModel($coreMenu, Constants::editAbility);
    ...
    public function update(UpdateCoreMenuRequest $request, $id)
    {
        try {
            $validatedData = $request->validated();

            $this->coreMenuService->update(
                id : $id,
                coreMenuData : $validatedData
            );
    ...
    public function destroy($id)
    {
        try {
            ...
            $this->handlePermissionWithModel($coreMenu, Constants::deleteAbility);

            $dataArr = $this->coreMenuService->delete($id);
    ...
    public function statusChange($id)
    {
        try {
            ...
            $status = $this->prepareStatusData($coreMenu);

            $this->coreMenuService->setStatus($id, $status);
    ...
    private function prepareIndexData($request)
    {
        ...
        $sub_menu_groups = $this->subMenuGroupService->getAll();

        // manipulate menu data
        $relations = ['core_sub_menu_group', 'owner', 'editor'];
        $coreMenus = CoreMenuWithKeyResource::collection($this->coreMenuService->getAll(relation : $relations,
            pagPerPage : $row,
            conds : $conds,
        ));

        // taking for column and columnFilterOption
        $columnAndColumnFilter = takingForColumnAndFilterOption(Constants::coreModule, $this->controlFieldArr());

        // prepare for permission
        $keyValueArr = [
            'createCoreModule' => 'create-coreModule',
        ];
    ...
    {
        $subMenuGroups = $this->subMenuGroupService->getAll();
        $icons = $this->iconService->getIcons();
        $modules = $this->moduleService->getAll(null, null, null, Constants::publish, Constants::yes);
        $coreFieldFilterSettings = $this->coreFieldService->getAll(code: Constants::coreModule,
