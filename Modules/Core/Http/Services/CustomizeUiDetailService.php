...
use Modules\Core\Transformers\Backend\Model\CustomFieldAttribute\CustomizeUiDetailWithKeyResource;
...
class CustomizeUiDetailService extends PsService
{
    ...
    public function __construct(CustomFieldService $customizeUiService)
    {
        $this->customizeUiService = $customizeUiService;
        $this->customizeUiCoreKeysIdCol = CustomField::coreKeysId;

        $this->customizeUiDetailIdCol = CustomFieldAttribute::id;
    ...
    {
        $customizeDetail = CustomFieldAttribute::when($id, function ($q, $data) {
    ...
    {
        // check permission
        $checkPermission = $this->checkPermission($this->viewAnyAbility, CustomFieldAttribute::class, "admin.index");
        ...
        $customizeHeader = $this->customizeUiService->get(null, null, null, $coreKeysId);
    ...
    {
        $checkPermission = $this->checkPermission($this->createAbility, CustomFieldAttribute::class, "admin.index");
    ...
    public function getCustomizeUiAndDetailNestedArray()
    {
        $customizeUisByModule = CustomField::where(CustomField::moduleName, Constants::item)
            ->latest()
            ->get();
        $customizeDetails = CustomFieldAttribute::whereIn(
            CustomFieldAttribute::coreKeysId,
            $customizeUisByModule->pluck(CustomFieldAttribute::coreKeysId)
        )
            ->get()
