...
class ItemService extends PsService
{
    ...
    public function __construct(protected PermissionServiceInterface $permissionService, FirebaseCloudMessagingService $firebaseCloudMessagingService, UserAccessApiTokenService $userAccessApiTokenService, PushNotificationTokenService $pushNotificationTokenService, BackendSettingService $backendSettingService, SearchHistoryService $searchHistoryService, UserService $userService, ImageService $imageService, CurrencyService $currencyService, LocationTownshipService $locationTownshipService, LocationCityService $locationCityService, CategoryServiceInterface $categoryService, SubcategoryService $subcategoryService, ShopService $shopService, BlockUserService $blockUserService, SystemConfigService $systemConfigService, UserInfoService $userInfoService, SubCatSubscribeService $subCatSubscribeService, VendorService $vendorService, CartItemService $cartItemService, LanguageService $languageService)
    {
    ...
    public function sendApprovalNoti($id)
    {
        ...
        foreach ($device_ids as $device_id) {
            $status = $this->firebaseCloudMessagingService->send_android_fcm($device_id, $data, $platform_names);
        }
        // send approval mail
        $to = $user->email;
        $subject = $subject;
    ...
    public function sendSubscribeNoti($item)
    {
        ...
        if ($updated_flag == 0) {
            ...
            if ($subcategory && $isSubscribed) {
                ...
                $status = $this->firebaseCloudMessagingService->send_android_fcm_topics_subscribe($data);
                ...
                $status = $this->firebaseCloudMessagingService->send_android_fcm_topics_subscribe_fe($data, $prj_name);
