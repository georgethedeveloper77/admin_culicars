...
class CoreAndCustomFieldService extends PsService
{
    ...
    public function __construct(CoreFieldFilterSettingService $coreFieldFilterSettingService, CustomizeUiService $customizeUiService, CategoryService $categoryService)
    {
        ...
        $this->screenDisplayUiModuleNameCol = DynamicColumnVisibility::moduleName;


        $this->coreFieldFilterModuleNameCol = CoreField::moduleName;
        $this->coreFieldFilterTableName = CoreField::tableName;
        $this->coreFieldFilterAddedDateCol = CoreField::addedDate;
        $this->coreFieldFilterUpdatedDateCol = CoreField::updatedDate;
        $this->coreFieldFilterUpdatedUserIdCol = CoreField::updatedUserId;
        $this->coreFieldFilterAddedUserIdCol = CoreField::addedUserId;
        $this->coreFieldFilterTableIdCol = CoreField::tableId;
        $this->coreFieldFilterIsDeleteCol = CoreField::isDelete;
        $this->coreFieldFilterIdCol = CoreField::id;
        $this->coreFieldFilterPlaceholderCol = CoreField::placeholder;
        $this->coreFieldFilterLabelNameCol = CoreField::labelName;
        $this->coreFieldFilterUpdatedFlagCol = CoreField::updatedFlag;



        $this->customUiIsDelCol = CustomField::isDelete;
        $this->customUiTableName = CustomField::tableName;
        $this->customUiTableIdCol = CustomField::tableId;
        $this->customUiIdCol = CustomField::id;
        $this->customUiNameCol = CustomField::name;
        $this->customUiPlaceholderCol = CustomField::placeholder;
        $this->customUiAddedDateCol = CustomField::addedDate;
        $this->customUiAddedUserIdCol = CustomField::addedUserId;
        $this->customUiUpdatedDateCol = CustomField::updatedDate;
        $this->customUiUpdatedUserIdCol = CustomField::updatedUserId;
        $this->customUiUpdatedFlagCol = CustomField::updatedFlag;
        ...
        try {
            ...
            if ($request->is_core_field == '1') {
                ...
                if ($field->is_include_in_hideshow == 1) {
                    ...
                    $screenDisplayUiSetting = DynamicColumnVisibility::where($this->screenDisplayUiModuleNameCol, $field->module_name)
                    ...
                    if ($screenDisplayUiSetting) {
                        ...
                        // save in screen_display_ui_setting
                        $screenDisplay = new DynamicColumnVisibility();
    ...
    function store($request, $tableId, $generatedData)
    {
        ...
        try {
            ...
            $checkCustomizeUi = CustomField::where("core_keys_id", $generatedData['core_keys_id'])->first();
            ...
            DynamicColumnVisibility::updateOrCreate(
                [
    ...
    public function index($request)
    {
        ...
        if (isset($conds['searchterm']) && $conds['searchterm']) {
            $customFields = DB::table($this->customUiTableName)
                ...
                // })
                ->select(
                    $this->languageStringTableName . '.' . $this->languageStringValueCol . ' as placeholder',
                    $this->customUiNameCol . ' as nameKey',
                    $this->customUiPlaceholderCol . ' as placeholderKey',
                    $this->languageStringValueCol . ' as name',
                    $this->customUiTableName . '.' . $this->customUiIdCol,
                    $this->customUiTableName . '.' . $this->customUiAddedDateCol,
                    $this->customUiTableName . '.' . $this->customUiAddedUserIdCol,
                    $this->customUiTableName . '.' . $this->customUiUpdatedDateCol,
                    $this->customUiTableName . '.' . $this->customUiUpdatedUserIdCol,
                    $this->customUiTableName . '.' . $this->customUiUpdatedFlagCol,
            ...
            $coreFields = DB::table($this->coreFieldFilterTableName)
                ...
                ->select(
                    $this->languageStringTableName . '.' . $this->languageStringValueCol . ' as placeholder',
                    $this->coreFieldFilterLabelNameCol . ' as nameKey',
                    $this->coreFieldFilterPlaceholderCol . ' as placeholderKey',
                    $this->languageStringValueCol . ' as name',
                    $this->coreFieldFilterTableName . '.' . $this->coreFieldFilterIdCol,
                    $this->coreFieldFilterTableName . '.' . $this->coreFieldFilterAddedDateCol,
                    $this->coreFieldFilterTableName . '.' . $this->coreFieldFilterAddedUserIdCol,
                    $this->coreFieldFilterTableName . '.' . $this->coreFieldFilterUpdatedDateCol,
                    $this->coreFieldFilterTableName . '.' . $this->coreFieldFilterUpdatedUserIdCol,
            ...
            $customFields = DB::table($this->customUiTableName)
                ...
                ->select(
                    ...
                    $this->customUiNameCol . ' as nameKey',
                    $this->customUiPlaceholderCol . ' as placeholderKey',
                    'lang.' . $this->languageStringValueCol . ' as name',
                    $this->customUiTableName . '.' . $this->customUiIdCol,
                    $this->customUiTableName . '.' . $this->customUiAddedDateCol,
                    $this->customUiTableName . '.' . $this->customUiAddedUserIdCol,
                    $this->customUiTableName . '.' . $this->customUiUpdatedDateCol,
    ...
    public function countCategory($categoryId = null, $tableId = null, $isCoreField = null, $isDelete = null)
    {
        if ($isCoreField || $isCoreField == 1) {
            $count = CoreField::when($tableId, function ($q, $tableId) {
            ...
            $count = CustomField::when($tableId, function ($q, $tableId) {
                $q->where('table_id', $tableId);
            })
        ...
        try {
            if ($request->isCoreField == '1') {
                ...
                if ($coreFields->is_include_in_hideshow == 1) {
                    ...
                    $screenDisplayUiSetting = DynamicColumnVisibility::where($this->screenDisplayUiModuleNameCol, $coreFields->module_name)
                ...
                if ($customField->is_include_in_hideshow == 1) {
                    ...
                    $screenDisplayUiSetting = DynamicColumnVisibility::where($this->screenDisplayUiModuleNameCol, $customField->module_name)
    ...
    public function syncFieldFromApi($request)
    {
        ...
        try {
            ...
            if ($request->is_core_field != '1') {
                if ($request->is_new == '1') {
                    $clientCustomizeUi = new CustomField();
                    ...
                    $clientCustomizeUi = CustomField::where('id', $request->id)->where('table_id', $tableId)->where('project_id', $projectId)->first();
                ...
                if ($clientCustomizeUi->is_include_in_hideshow == 1) {
                    ...
                    $screenDisplayUiSetting = DynamicColumnVisibility::where($this->screenDisplayUiModuleNameCol, $clientCustomizeUi->module_name)
                ...
                if ($request->is_new == '1') {
                    ...
                    $coreField = CoreField::where('id', $request->id)->where('table_id', $tableId)->where('project_id', $projectId)->first();
                    if (!$coreField) {
                        DB::rollBack();
                        return responseMsgApi('overwrite', $this->badRequestStatusCode);
                    }
                ...
                if ($coreField->is_include_in_hideshow == 1) {
                    ...
                    $screenDisplayUiSetting = DynamicColumnVisibility::where($this->screenDisplayUiModuleNameCol, $coreField->module_name)
