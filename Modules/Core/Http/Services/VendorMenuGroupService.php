 ...
 * @deprecated
 */
class VendorMenuGroupService extends PsService
{
    ...
    public function __construct()
    {
        ...
        $this->menuGroupIsShowOnMenuCol = VendorMenuGroup::isShowOnMenu;
    ...
    public function getMenuGroups($relation = null, $pagPerPage = null, $conds = null)
    {
        ...
        $menu_groups = VendorMenuGroup::when(isset($conds['order_by']) && $conds['order_by'], function ($q) use ($conds) {
    ...
    {
        $menu_group = VendorMenuGroup::when($id, function ($q, $id) {
            $q->where('id', $id);
        })
            ->when($relation, function ($q, $relation) {
                $q->with($relation);
    ...
    public function index($request)
    {
        ...
        $checkPermission = $this->checkPermission($this->viewAnyAbility, VendorMenuGroup::class, "admin.index");
        ...
        $menu_groups = VendorMenuGroupWithKeyResource::collection($this->getMenuGroups($relations, $row, $conds));
    ...
    public function create()
    {
        ...
        $coreFieldFilterSettings = $this->getCoreFieldFilteredLists($code);
        // check permission start
        $checkPermission = $this->checkPermission($this->createAbility, VendorMenuGroup::class, "admin.index");
        // check permission end
        $dataArr = [
