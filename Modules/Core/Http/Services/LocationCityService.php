...
class LocationCityService extends PsService
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $this->locationCityInfoServiceInterface->save($request, $city->id);
                    ...
                    foreach ($cityRelations as $key3 => $cityRelation) {
                        ...
                        if ($coreKeysIdFromReq == $cityRelation->core_keys_id) {
                            ...
                            if (is_file($valueFromReq)) {
                                ...
                                delImageFromCustomFieldValue($cityRelation, Constants::storageOriginalPath, Constants::storageThumb1xPath, Constants::storageThumb2xPath, Constants::storageThumb3xPath);
                            ...
                            $customizeHeaders = CustomField::where('module_name', $this->code)->get();
                            foreach ($customizeHeaders as $key2 => $customizeHeader) {
                                if ($coreKeysIdFromReq === $customizeHeader->core_keys_id) {
                                    $cityRelation->ui_type_id = $customizeHeader->ui_type_id;
                                    $cityRelation->core_keys_id = $customizeHeader->core_keys_id;
    ...
    public function searching($query, $conds)
    {
        ...
        if (isset($conds['city_relation']) && !empty($conds['city_relation'])) {
            $customizeUis = CustomField::where($this->customUiModuleName, 'loc')->latest()->get();
    ...
    public function index($routeName, $request)
    {
        ...
        // $code = $this->code;
        $customizeUis = CustomField::where($this->customUiModuleName, 'loc')->where($this->customUiUiTypeIdCol, 'uit00001')->latest()->get();
        $cityRelations = [];
        //        $uis = [];
        ...
        $customizeUisByModule = CustomField::where($this->customUiModuleName, 'loc')->latest()->get();
