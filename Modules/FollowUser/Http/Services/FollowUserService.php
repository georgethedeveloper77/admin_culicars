...
{
    protected $firebaseCloudMessagingService, $followerReturnType, $followingReturnType, $noContentStatusCode, $successStatus, $coreKeyService, $usrFollowerCount, $usrFollowingCount, $userService, $createdStatusCode, $internalServerErrorStatusCode, $okStatusCode, $userId, $followedUserId, $deviceTokenParaApi, $loginUserIdParaApi, $userAccessApiTokenService, $forbiddenStatusCode;

    public function __construct(FirebaseCloudMessagingService $firebaseCloudMessagingService, CoreKeyService $coreKeyService, UserService $userService, ItemService $itemService, PushNotificationTokenService $pushNotificationTokenService, UserAccessApiTokenService $userAccessApiTokenService)
    {
    ...
    public function followUserFromApi($request)
    {
        ...
        if (empty($userAccessApiToken)) {
            ...
            foreach ($device_ids as $device_id) {
                $this->firebaseCloudMessagingService->send_android_fcm($device_id, $data, $platform_names);
            }
    ...
    public function updateUserFollowerCountWithId($user_id)
    {
        ...
        if ($follower) {
            ...
            $saveFollower->ui_type_id = CustomField::where($this->customUiCoreKeysIdCol, $this->usrFollowerCount)->first()->ui_type_id;
        ...
        if ($following) {
            ...
            $saveFollowing->ui_type_id = CustomField::where($this->customUiCoreKeysIdCol, $this->usrFollowingCount)->first()->ui_type_id;
