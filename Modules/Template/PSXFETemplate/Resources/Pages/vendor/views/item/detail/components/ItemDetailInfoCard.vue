<template>
            ...
            <div v-for="pr in productRelation" :key="pr.id" class="">
                <ps-label
                    textColor="text-base font-medium text-feSecondary-800 dark:text-feSecondary-50">{{
                        $t(itemCustomFieldStore.customField.data?.customList.filter((cf) => cf.coreKeysId ==
    ...
    <div class="col-span-8 mt-6">
        <div class="bg-feSecondary-50 dark:bg-feSecondary-800 rounded-lg px-2 py-4 lg:p-4 flex flex-col">
            ...
            <div v-for="productRelation in productStore.item?.data?.productRelation.filter((pr) => pr.coreKeysId != 'ps-itm00002' && pr.coreKeysId != 'ps-itm00004' && pr.coreKeysId != 'ps-itm00007' && pr.coreKeysId != 'ps-itm00008' && pr.coreKeysId != 'ps-itm00046')"
                :key="productRelation.id">
            ...
            <div v-if="productStore.item?.data?.phone != null && productStore.item?.data?.phone != ''">
                <div v-if="loginUserId == PsConst.NO_LOGIN_USER">
                    <ps-route-link :to="{ name: 'login' }">
                        <ps-label textColor="text-base cursor-pointer font-medium text-fePrimary-500 mt-6">
                            {{ $t('core_fe__login_to_see_contact_number') }}
                        </ps-label>
                    </ps-route-link>
                </div>
                <div v-else>
