<template>
    ...
    <div @click="clickedButton"
        ...
        :class="{ 'justify-start': sideMenuStore.isFullSideMenu, 'sm:justify-center': !sideMenuStore.isFullSideMenu,    [activeTheme] : $t(sideMenuStore.sidebarActive) == name, [defaultTheme] : $t(sideMenuStore.sidebarActive) != name, [rounded] : true, [padding] : true,  }">
        <div class="relative items-center ps-10">
            <!-- <ps-icon v-if="showIcon" :name="icon" viewBox="0 0 24 24" w="24" h="24" /> -->
            <span :class="!sideMenuStore.isFullSideMenu ? 'sm:hidden' : ''" class="text-base me-2">
                {{ name }}
            </span>
        </div>
