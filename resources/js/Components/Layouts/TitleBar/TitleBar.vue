  ...
  >
    ...
    <div class="flex items-center text-secondary-800 ">
        ...
        <ps-dropdown horizontalAlign="right" class='w-full ms-3 sm:ms-4 lg:ms-6 xxl:ms-8' h="h-auto">
            <template #select>
                <ps-icon @click="clickMessageButton" name="email" class='cursor-pointer text-secondary-800 dark:text-secondary-100 text-'/>
            </template>
            <template #list>
                    ...
                    <div class="z-30 py-2">
                        <div class="items-center w-full px-4 py-2">
                            ...
                            <div class="flex justify-between">
                                ...
                                <ps-label v-if="$page.props.can.updateContact" @click="makeAllRead" class="text-xs cursor-pointer sm:text-sm" textColor="text-primary-500">{{$t('core__be_mark_all_as_read')}}</ps-label>
                            </div>
                        ...
                        <div v-if="loading">
                            <ps-label class="py-6 text-center"> {{$t('core__be_loading')}} </ps-label>
                        </div>

                        <div v-else-if="contacts.data?.length > 0" class="overflow-y-auto h-80">
                            <div v-for="contact in contacts.data" :key="contact.id" class="cursor-pointer">
                                <div :class="contact.is_read == 0 ? 'bg-primary-50 dark:bg-primary-900': 'bg-secondary-50 dark:bg-secondary-900 '" class="inline-flex items-center w-full px-4 py-3 mb-2 text-base rounded shadow-sm">
                                    <div class="flex flex-row items-center w-full" @click="goToDetail(contact.id)">
                                        <!-- <template #content> -->
                                        <!-- <img class="inline-block w-8 h-8 mx-2 rounded-full ring-2 ring-white" src="https://images.unsplash.com/photo-1491528323818-fdd1faba62cc?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt=""/> -->
                                        <div class="w-8 h-8 sm:w-12 sm:h-12">
                                            ...
                                            v-lazy=" { src: $page.props.uploadUrl + '/' + contact.owner?.user_cover_photo, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_profile.png' }"
                                             width="20" height="20"
                                                    class="object-cover w-8 h-8 mx-2 rounded-full sm:w-12 sm:h-12 ring-2" alt="Profile photo"/>
                                            <!-- <img v-else :src="defaultProfileImg" width="20" height="20"
                                                    class="object-cover w-8 h-8 mx-2 rounded-full sm:w-12 sm:h-12 ring-2" alt="Profile photo" @error="defaultProfileImg" /> -->
                                        </div>
                                        <span class="w-full ms-4">
                                            <span class="flex flex-col grid-cols-2 text-sm sm:flex-row sm:justify-between lg:justify-start lg:grid ">
                                                <ps-label class="text-xs font-semibold sm:text-sm">{{contact.contact_name}}</ps-label>
                                                <ps-label-caption-3>{{$t('core__be_by')}} : {{contact.contact_email}}</ps-label-caption-3>
                                            </span>
                                            <div class="overflow-hidden truncate w-44 sm:w-68 lg:w-104">
                                                <ps-label-caption-3 class="truncate w-44 sm:w-68 lg:w-104">{{contact.contact_message}}</ps-label-caption-3>
                                            </div>
                                        </span>
                                        <!-- </template> -->
                                    </div>
                                    <ps-icon v-if="contact.authorization.delete" name="close" @click="clickedDeleteContact(contact.id)" class="justify-end my-auto text-sm text-secondary-400 ms-auto focus:shadow-none hover:text-purple-500"  />
                                </div>
                            </div>
                        </div>

                        <div v-else>
                            <ps-label  class="py-6 text-center"> {{$t('core__be_no_contact_message')}} </ps-label>
                        </div>

                        <ps-text-button class="justify-center w-full m-3" textColor="text-primary-500" @click="toContact()">{{$t('core__be_view_all_message')}}</ps-text-button>
                    </div>
                </div>
            </template>
        </ps-dropdown>
            ...
            <template #list>
                <div class="w-56 rounded-md shadow-xs ">
                    <div class="z-30 ">
                        ...
                        <Link :href="route('vendor.setSession')" v-if="$page.props.canAccessVendor"
                            class="flex items-center w-56 p-4 cursor-pointer hover:bg-primary-000 dark:hover:bg-primary-900">
                            <ps-icon name="refresh"  />
                            <ps-label class="ms-2">{{$t('core__be_switch_to_vendor')}}</ps-label>
                        </Link>
...
export default {
  ...
  setup (){
    ...
    function handleLanguage(row){
        // this.$inertia.put(route('language.changeLanguage',row.symbol));
